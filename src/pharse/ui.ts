...
import {
  BALANCE_RELOAD_INTERVAL,
  PIPES_AVAILABLE,
  PIPES_COSTS,
  SHOP_RELOAD_INTERVAL,
} from "./consts";
import { Config } from "./config";

export class UI {
  scoreDiv: HTMLDivElement = document.getElementById("score") as HTMLDivElement;
  rewardsDiv: HTMLDivElement = document.getElementById(
    "rewards"
  ) as HTMLDivElement;
  spinnerDiv: HTMLDivElement = document.getElementById(
    "spinner-container"
  ) as HTMLDivElement;
  skinChooserDiv: HTMLDivElement = document.getElementById(
    "skin-chooser"
  ) as HTMLDivElement;
  skinPrevDiv: HTMLDivElement = document.getElementById(
    "skin-prev"
  ) as HTMLDivElement;
  skinCurrentDiv: HTMLDivElement = document.getElementById(
    "skin-current"
  ) as HTMLDivElement;
  skinImage: HTMLImageElement = document.getElementById(
    "skin-image"
  ) as HTMLImageElement;
  skinNextDiv: HTMLDivElement = document.getElementById(
    "skin-next"
  ) as HTMLDivElement;
  useButton: HTMLButtonElement = document.getElementById(
    "use"
  ) as HTMLButtonElement;
  shopButton: HTMLButtonElement = document.getElementById(
    "shop"
  ) as HTMLButtonElement;
  playButton: HTMLButtonElement = document.getElementById(
    "play"
  ) as HTMLButtonElement;
  buttonsDiv: HTMLDivElement = document.getElementById(
    "buttons"
  ) as HTMLDivElement;
  balanceDiv: HTMLDivElement = document.getElementById(
    "balance"
  ) as HTMLDivElement;
  playTextDiv: HTMLDivElement = document.getElementById(
    "play-text"
  ) as HTMLDivElement;
  useTextDiv: HTMLDivElement = document.getElementById(
    "use-text"
  ) as HTMLDivElement;
  balanceContainerDiv: HTMLDivElement = document.getElementById(
    "balance-container"
  ) as HTMLDivElement;
  afterGameDiv: HTMLDivElement = document.getElementById(
    "after-game"
  ) as HTMLDivElement;
  errorDiv: HTMLDivElement = document.getElementById("error") as HTMLDivElement;
  tokensAwardedDiv: HTMLDivElement = document.getElementById(
    "tokens-awarded"
  ) as HTMLDivElement;
  newAchievementsDiv: HTMLDivElement = document.getElementById(
    "new-achievements"
  ) as HTMLDivElement;
  ...
  constructor(public readonly config: Config, public readonly gameFi: GameFi) {
    ...
    this.useButton.addEventListener("click", () => {
      if (
        this.previewPipeIndex !== 0 &&
        this.purchases.findIndex(
          (p) => p.systemName === this.getPreviewPipe()
        ) === -1
      ) {
      ...
      window.localStorage.setItem(
        "chosen-pipe",
        this.currentPipeIndex.toString()
      );
      this.redrawShop();
    });
  ...
  async getBalance() {
    try {
      const jetton = this.gameFi.openJetton(
        Address.parse(this.config.TOKEN_MASTER)
      );
      const jettonWallet = await jetton.getWallet(
        Address.parse(this.gameFi.wallet.account.address)
      );
  ...
  async buy(itemId: number) {
    ...
    this.gameFi.buyWithJetton({
      ...
      forwardPayload:
        (window as any).Telegram.WebApp.initDataUnsafe.user.id + ":" + itemId,
    });
  }
  ...
  showMain(
    again: boolean,
    results?: { reward: 0; achievements: string[] } | { error: string }
  ) {
    if (again) {
  ...
  redrawShop() {
    ...
    this.skinPrevDiv.style.display =
      this.previewPipeIndex > 0 ? "unset" : "none";
    this.skinNextDiv.style.display =
      this.previewPipeIndex < PIPES_AVAILABLE.length - 1 ? "unset" : "none";
    const bought =
      this.purchases.findIndex((p) => p.systemName === this.getPreviewPipe()) >=
      0;
    if (this.previewPipeIndex === this.currentPipeIndex) {
      ...
      this.useTextDiv.innerText =
        "Buy for " + PIPES_COSTS[this.previewPipeIndex];
  ...
  async reloadPurchases() {
      ...
      const purchasesData = await (
        await fetch(
          this.config.ENDPOINT +
            "/purchases?auth=" +
            encodeURIComponent((window as any).Telegram.WebApp.initData)
        )
      ).json();
    ...
    this.reloadShopTimeout = window.setTimeout(
